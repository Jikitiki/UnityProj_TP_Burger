...
public class rectRay : MonoBehaviour
{
    ...
    void Update()
    {
        // SI Click + raycast et raycast != sol + object d�tect� 0 enfant
        if (Input.GetMouseButtonUp(0) && (Physics.Raycast(transform.position, transform.forward, out hit, _maxDist) && hit.collider.gameObject.tag != "floor") && hit.collider.gameObject.GetComponent<Transform>().childCount < 1)
        {
            Debug.Log("Touch�! ("+hit.collider.gameObject.name+")"+"   --  childCount : "+ hit.collider.gameObject.GetComponent<Transform>().childCount);
